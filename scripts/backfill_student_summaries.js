...
// Helper functions (copied from EvaluationRouter for script standalone usage)
function toDateRange(startDate, endDate) {
    const start = new Date('2026-03-26T00:00:00.000Z');
    const end = new Date('2026-03-29T23:59:59.999Z');
    return { start, end };
}
...
async function runBackfill() {
    ...
    try {
        ...
        for (const student of students) {
            ...
            const payload = toSummaryPayload(student.id, summary);
            ...
            if (error) {
                console.error(`  [ERROR] Failed to upsert ${student.name}:`, error.message);
            } else {
